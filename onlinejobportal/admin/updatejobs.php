...
if(isset($_POST["id"]) && !empty($_POST["id"])){
   // Get hidden input value
    ...
    //     $image_sucess =  "The file ". basename( $_FILES["fileToUpload"]["name"]). " has been uploaded.";
    //   } else {
    ...
    if(empty($title_err) && empty($company_err) && empty($experience_err) && empty($description_err) && empty($opening_err)  && empty($location_err)){
        ...
        $sql = "UPDATE jobs SET title=?, company=?, experience=?, description=?, opening=? ,  location=? WHERE id=?";
        if($stmt = mysqli_prepare($con, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "ssssssi", $param_title, $param_company, $param_experience , $param_description, $param_opening,$param_location,$param_id);
                    ...
                    <form action="<?php echo htmlspecialchars(basename($_SERVER['REQUEST_URI'])); ?>" method="post" enctype="multipart/form-data">
                       <div class="form-group <?php echo (!empty($title_err)) ? 'has-error' : ''; ?>">
                        ...
                        <!--  <div class="form-group <?php echo (!empty($image_err)) ? 'has-error' : ''; ?>">
                            ...
                            <img width ='80px' src="<?php echo $image;?>"/>
